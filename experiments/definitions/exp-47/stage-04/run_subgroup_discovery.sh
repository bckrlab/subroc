...
NOTEBOOK_PATH="$EXPERIMENT_CODE_PATH/notebooks/03_experiments/exp-2_subgroup_discovery.ipynb"
PARENT_PATH="$(dirname "${BASH_SOURCE[0]}")"
STAGE_NAME="$(basename $PARENT_PATH)"

# avoid race conditions when notebooks create this directory
mkdir -p "$STAGE_OUTPUT_PATH/data/processed/uci_breast_cancer_wisconsin_original"
